#!/usr/bin/env python3
"""Build the P1 public-GitHub pilot dataset + the issue->fixing-PR benchmark.
...
snapshot and re-runs the (deterministic) ablation on it.
...
Run (needs a token):
    GITHUB_TOKEN=$(gh auth token) python data/pilot/build_pilot.py
"""
...
def main() -> None:
    ...
    _write_dataset_card(records, edges, queries, retrieved_at)

    print(f"\nPILOT: {len(records)} records, {len(edges)} fixes edges, "
...
def _write_source_cards(licenses: dict, retrieved_at: str) -> None:
    ...
    for repo, lic in sorted(licenses.items()):
        rows.append({
            ...
            "transform": "python data/pilot/build_pilot.py",
            "redistribution": "metadata_only",
        })
    _write_jsonl(HERE / "source-cards.jsonl", rows)


def _write_dataset_card(records, edges, queries, retrieved_at) -> None:
    card = {
        "card_type": "dataset",
        "id": "ds:gh-pilot-v0",
        "name": "public GitHub issue->fixing-PR pilot",
        "version": "v0",
        "created_at": retrieved_at,
        "sources": [f"src:gh:{r}" for r in REPOS],
        ...
        "notes": "Built by data/pilot/build_pilot.py (live GitHub REST). "
                 "Hard negatives are same-repo PRs by title/body token overlap.",
    }
    _write_json(CARDS / "gh-pilot-v0.dataset-card.json", card)
