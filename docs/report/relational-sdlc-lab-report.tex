...
structure), not as a post-hoc head. We close with the next steps: scaling the win
with multi-split confidence intervals, a code-specific base, graph link
prediction, and --- only after MVP-1 holds --- a relational small language model.
\end{abstract}
...
This is a standalone synthesis of the public repository's work to date. It is
...
\begin{description}[leftmargin=1.4em,style=nextline]
\item[Scale the win (Track D).] Confirm the LoRA result with multi-split confidence
intervals and multiple seeds; expand from the 20-repo pilot toward Tier-2 (200--500
repos); and try a \emph{code-specific} base embedder (Q6). Do not scale before signal
--- the signal is now present.
