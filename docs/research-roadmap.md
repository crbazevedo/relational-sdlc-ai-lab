...
| **diff→test leakage audit (R23)** | is the diff→test number a densification / candidate-coverage artefact? | **no — passes** — candidate coverage is balanced (gold 85.7% vs negatives 81.9%); fair R@1 (ranking only among *covered* candidates) is 0.500 = **5.7× random**; train/test repos disjoint (no fork/near-dup) | overall 0.429 is *conservative* vs fair 0.500; signal = real discrimination | `diff2test-audit-results.json` |
| **external validity (R24)** | does diff→test replicate on an independent, different-language corpus? | **yes — replicates** — built a 12-repo TS/JS corpus (4,962 `modifies` edges, disjoint from the Python pilot); same release-honest method; the density-response recurs (sparse at shallow ingest, denser with history) | **fair R@1 5.5× random vs pilot 5.7×** (near-identical, pool-/reachability-invariant); R@1 0.351, balanced coverage (gold 54.1% vs neg 57.8%) | `corpus2-diff2test-results.json` |
| **baselines + refutation (R25)** | do the `[PENDING]` baselines (BM25) change the picture? | **YES — the diff→test headline is REFUTED.** BM25 over test **paths** beats co-change structure on both corpora (paths are text; "text-free" was wrong). A learned lexical reranker is best (0.707); adding structure *hurts* it (ΔR@1 −0.038, CI [−0.066,−0.009]); naive fusion hurts. Task A survives but its LoRA win is **underpowered** (Δ+0.063 < MDE 0.080, power 0.60) | Task B: BM25 **0.536** (pilot) / **0.609–0.707** (corpus2) vs structure 0.348–0.429. Task A: BM25 0.500 < embedder 0.592 < LoRA 0.655 < +graph 0.690 | `baselines-metrics-results.json`, `corpus2-baselines-results.json`, `corpus2-fusion-results.json` |
| **label-precision audit (R26)** | are the "verifiable" relations actually clean? (last `[PENDING]` item) | **yes — sound** — construction precision **99.4%** `fixes` (closing-keyword-verified), **100%** `modifies` (git-history); manual sample confirms `fixes` is semantically correct even at ~0 title-overlap (the lexical proxy understates it). 29% of modify-edges target tests (composition, not precision) | `fixes` 354/356, `modifies` 1356/1356; title-Jaccard median 0.188 (88% >0) | `label-audit-results.json` |
| **static SE baseline (R27)** | does a static change-based test selector (the field's baseline, since Ekstazi/STARTS are JVM-only) also beat structure? | **yes — negative result hardened** — changed-source-path→test-path proximity (history-free, uses only the diff) beats co-change structure; best static system (proximity+BM25) dominates it. MSR-primary venue chosen; this is the SE-baseline reviewers demand | path-proximity R@1 **0.580**, best-static **0.679** vs structure 0.429 (same 112-q pilot harness, 91% source coverage) | `path-proximity-results.json` |
