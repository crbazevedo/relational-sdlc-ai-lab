#!/usr/bin/env python3
"""LoRA-at-Tier-2: frozen + LoRA-tuned MiniLM embeddings of the dense ~80-repo set.

Confirms whether the LoRA win tightens/holds with DENSER per-repo coverage (median
~33 fixes-queries/repo, up from ~16). Same recipe as the pilot/scale (LoRA r=8 on
attention q/k/v, symmetric InfoNCE on TRAIN-repo fixes pairs only, eval held-out
repos). Caches both embeddings (gitignored); eval is numpy-only.

Needs the [embed] extra. Run:  python data/tier2/finetune_tier2.py
"""
...
def main() -> None:
    ...
    # Reuse a valid frozen cache if present (the frozen embed is ~7 min and
    # deterministic) so a re-run only redoes the LoRA half.
    if (EMB / "minilm-l6-v2.npz").exists():
        print("frozen cache present — skipping frozen embed", file=sys.stderr, flush=True)
    else:
        ...
        print("wrote frozen cache", file=sys.stderr, flush=True)
